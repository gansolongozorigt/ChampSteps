...
function Dashboard() {
  ...
  async function handleDownloadPdf(template?: PdfTemplate) {
    ...
    setPdfBusy(true);
    try {
      await exportPortfolio(child, achievements, { t, template: template ?? pdfTemplate });
  ...
  // Хязгаарын анхааруулга: maxAchievements-н 80%-д хүрэхэд
  ...
  const navItems: { id: NavSection; label: string; icon: React.ReactNode; activeIcon: React.ReactNode; color: string }[] = [
    {
      id: "achievements", label: t("nav.achievements") || "Амжилт", color: "text-amber-500",
      ...
      activeIcon: <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path fillRule="evenodd" d="M5.166 3.073A.75.75 0 016 2.25h12a.75.75 0 01.834.823l-1.664 9.988a.75.75 0 01-.74.64H7.57a.75.75 0 01-.74-.64L5.166 3.897a.75.75 0 010-.824zM5.25 20.25a.75.75 0 01.75-.75h12a.75.75 0 010 1.5H6a.75.75 0 01-.75-.75zm1.5-3a.75.75 0 01.75-.75h9a.75.75 0 010 1.5h-9a.75.75 0 01-.75-.75z" clipRule="evenodd"/></svg>,
    },
    {
      id: "practice", label: t("nav.practice") || "Бэлтгэл", color: "text-blue-500",
      ...
      activeIcon: <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path fillRule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zM12.75 6a.75.75 0 00-1.5 0v6c0 .414.336.75.75.75h4.5a.75.75 0 000-1.5h-3.75V6z" clipRule="evenodd"/></svg>,
    },
    {
      id: "reflection", label: t("nav.reflection") || "Сэтгэл", color: "text-rose-500",
      ...
      activeIcon: <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M11.645 20.91l-.007-.003-.022-.012a15.247 15.247 0 01-.383-.218 25.18 25.18 0 01-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0112 5.052 5.5 5.5 0 0116.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 01-4.244 3.17 15.247 15.247 0 01-.383.219l-.022.012-.007.004-.003.001a.752.752 0 01-.704 0l-.003-.001z"/></svg>,
    },
    {
      id: "coach", label: t("nav.coach") || "Багш", color: "text-emerald-500",
    ...
    {
      id: "ai", label: t("nav.ai") || "AI", color: "text-violet-500",
      icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0112 15a9.065 9.065 0 00-6.23-.693L5 14.5m14.8.8l1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0112 21a48.309 48.309 0 01-8.135-1.587c-1.718-.293-2.3-2.379-1.067-3.61L5 14.5"/></svg>,
      activeIcon: <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path fillRule="evenodd" d="M8.25 2.25A.75.75 0 019 3v.75h2.25V3a.75.75 0 011.5 0v.75H15V3a.75.75 0 011.5 0v.75h.75a3 3 0 013 3v.75H21A.75.75 0 0121 9h-.75v2.25H21a.75.75 0 010 1.5h-.75V15H21a.75.75 0 010 1.5h-.75v.75a3 3 0 01-3 3h-.75V21a.75.75 0 01-1.5 0v-.75h-2.25V21a.75.75 0 01-1.5 0v-.75H9V21a.75.75 0 01-1.5 0v-.75h-.75a3 3 0 01-3-3v-.75H3A.75.75 0 013 15h.75v-2.25H3a.75.75 0 010-1.5h.75V9H3a.75.75 0 010-1.5h.75v-.75a3 3 0 013-3h.75V3a.75.75 0 01.75-.75zM6 6.75A.75.75 0 016.75 6h10.5a.75.75 0 01.75.75v10.5a.75.75 0 01-.75.75H6.75a.75.75 0 01-.75-.75V6.75z" clipRule="evenodd"/></svg>,
    },
  ];
      ...
      <header className="sticky top-0 z-40 bg-stone-950 print:hidden">
        <div className="px-4 py-2.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Tier badge — бүх tier-д харагдана */}
          ...
          <div className="flex items-center gap-1.5">
            ...
            {/* Avatar — premium dot + профайл */}
            <button
              onClick={() => setShowProfile(true)}
              className="relative w-8 h-8 rounded-full overflow-visible border-2 border-stone-700 hover:border-amber-500 transition-colors shrink-0"
              title="Профайл"
            >
              ...
              {isPremium && tier.dotColor && (
                <span
                  className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-stone-950"
            ...
            <div className="flex-1 min-w-0">
              {showLimitWarning ? (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span className="text-[10px] font-semibold text-amber-400">
                        {achCount}/{maxAch} амжилт
                      </span>
                      <span className="text-[9px] text-stone-500">хязгаарт ойртлоо</span>
                    </div>
                    <div className="h-1 bg-stone-800 rounded-full overflow-hidden w-24">
                      <div
                ...
                <span className="text-[10px] text-stone-500">
                  <span className="text-stone-400 font-medium">Үнэгүй</span> эрх · {achCount}/{maxAch} амжилт
                </span>
              )}
            </div>
            <button
              onClick={() => setShowSubscription(true)}
              className="flex items-center gap-1 text-[10px] font-bold px-2.5 py-1.5 rounded-md bg-amber-500 text-stone-950 hover:bg-amber-400 active:scale-95 transition-all shrink-0"
            >
              ⬆ Upgrade
            </button>
          </div>
            ...
            return (
              <button
                ...
                className={`relative flex-1 flex flex-col items-center justify-center gap-0.5 py-2.5 transition-colors ${
                  active ? item.color : "text-stone-400 hover:text-stone-500"
                }`}
              >
                <span className={`transition-transform ${active ? "scale-110" : ""}`}>
                  {active ? item.activeIcon : item.icon}
                </span>
                <span className={`text-[9px] font-medium leading-none ${active ? item.color : "text-stone-400"}`}>
                  {item.label}
                </span>
                {active && (
                  <span className={`absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full ${
                    item.id === "achievements" ? "bg-amber-500" :
                    item.id === "practice"    ? "bg-blue-500" :
                    item.id === "reflection"  ? "bg-rose-500" :
                    item.id === "coach"       ? "bg-emerald-500" :
                    "bg-violet-500"
                  }`}/>
                )}
              </button>
