...
function Dashboard() {
  ...
  // Tier тохиргоо — badge өнгө, label
  ...
  // Хязгаарын анхааруулга: 80%-д хүрэхэд
    ...
    <div className="flex flex-col min-h-screen bg-stone-100 font-sans">
      ...
      <header className="sticky top-0 z-40 bg-stone-950 print:hidden">
          ...
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              {isPremium && tier.dotColor && (
                <span className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-stone-950" style={{ background: tier.dotColor }}/>
              )}
            </button>
          </div>
        </div>
        ...
        <div className="bg-stone-900 px-3 pb-2 flex items-center gap-2 overflow-x-auto scrollbar-hide border-b border-stone-800">
          ...
          {/* Subscription товч — tab мөрийн баруун талд */}
          <button
          ...
          >
            ...
            {isPremium ? tier.label : t("nav.upgrade") || "Pro"}
          </button>
        </div>
      </header>
      ...
      <nav className="fixed bottom-0 inset-x-0 z-40 bg-white border-t border-stone-200 print:hidden" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span className="text-[10px] font-semibold text-amber-400">{achCount}/{maxAch} амжилт</span>
                    ...
                    <div className="h-1 bg-stone-800 rounded-full overflow-hidden w-24">
                      <div className="h-full rounded-full bg-gradient-to-r from-amber-500 to-red-500 transition-all" style={{ width: `${Math.min(100, (achCount / maxAch) * 100)}%` }}/>
                    </div>
                  </div>
                </div>
              ) : (
                <span className="text-[10px] text-stone-500">
                  <span className="text-stone-400 font-medium">ҮНЭГҮЙ</span> · {achCount}/{maxAch} амжилт
                </span>
              )}
            </div>
            <button
        ...
        {/* Nav items — зөвхөн өнгө өөрчлөгдөнө, icon хэвтэй */}
              ...
              >
                ...
                {active && (
                  <span className={`absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full ${
                    ...
                    item.id === "coach"       ? "bg-emerald-500" : "bg-violet-500"
                  }`}/>
                )}
              </button>
            );
