...
export default function AchievementSummary({ achievements, }) {
    ...
    return (_jsxs("section", { className: "rounded-2xl border border-stone-200 bg-white p-5 mb-5 shadow-sm", children: [_jsx("p", { className: "text-center text-[13px] text-stone-500 mb-4", children: t("summary.totalAchievements", { n: total }) }), _jsx("div", { className: "grid grid-cols-4 gap-1.5 sm:gap-3", children: MEDALS.map((m, i) => (_jsx(MedalItem, { emoji: m.emoji, label: t(`awards.${m.key}`), value: counts[m.key] ?? 0, index: i }, m.key))) })] }));
}
function MedalItem({ emoji, label, value, index, }) {
    const n = useCountUp(value);
    return (_jsxs("div", { className: "flex flex-col items-center gap-0.5 sm:gap-1 rounded-xl py-2 sm:py-2.5", children: [_jsx("span", { className: "text-[26px] sm:text-[38px] leading-none animate-pop", style: { animationDelay: `${index * 0.09}s` }, children: emoji }), _jsx("span", { className: "text-[16px] sm:text-[20px] font-bold text-stone-900 leading-tight", children: n }), _jsx("span", { className: "text-[10px] sm:text-[12px] font-medium text-stone-500 text-center leading-tight", children: label })] }));
}
